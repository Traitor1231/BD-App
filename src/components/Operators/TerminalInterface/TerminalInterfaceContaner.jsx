  ...
  <StoreContext.Consumer>
    {(store) => {
      ...
      const SumValidateMaxValue = () => {
        ...
        if (SumInputRef.current.value[0].includes(RubleSumbol)) {
          SumInputRef.current.value = SumInputRef.current.value.replace(
            RubleSumbol,
            '',
          );
      ...
      const AddRubleSymbol = () => {
        ...
        if (!SumInputRef.current.value.includes(RubleSumbol)) {
          SumInputRef.current.value = SumInputRef.current.value + RubleSumbol;
        }
        SumInputRef.current.selectionEnd = SumInputRef.current.selectionStart - 1;
      };
      ...
      const PhoneValidateValue = (e) => {
        if (!e.target.validity.valid) {
          ErrorTextRef.current.innerHTML = 'Введите корректный адрес телефона';
        }
        e.preventDefault();
      };
      const FormValidate = (e) => {
        ...
        if (PhoneInputRef.current.value === '') {
          ...
          SumInputRef.current.value > 1000
              || SumInputRef.current.value < 1
        ) {
