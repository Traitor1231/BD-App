        ...
        <StoreContext.Consumer>
            {(store: any) => {
                ...
                const AddRubleSymbol = ():void => {
                    ...
                    if (!GetSumValue().includes(RubleSumbol)) SetRubbleSumbolToStringEnd();
                    ChangeSelectionEnd();
                };
                const DeleteRubleSymbol = ():void => {
                    if (GetSumValue().includes(RubleSumbol)) ReplaceSumbols(RubleSumbol, '');
                    ChangeInputType('number');
                };
                const PhoneValidateValue = (e: { target: { validity: { valid: boolean; }; }; preventDefault: () => void; }):void => {
                    if (!e.target.validity.valid) SetErrorMessage('Введите корректный номер телефона');
                    e.preventDefault();
                };
                ...
                const isRedirect = ():boolean => (
                    Math.round(Math.random()) === 1 ? isFormSubmit(store.RejectData, SetErrorMessage('Оплата произошла успешно'), true,) :
                        isFormSubmit(store.SubmitData, SetErrorMessage('При оплате произошла ошибка, попробуйте позже'), false,)
                );
